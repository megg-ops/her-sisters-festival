...
- **墨靛（indigo-ink）**：任务面板、输入框、文化来源抽屉等主要 UI 表面。
...
**The No Parchment Rule.** 禁止用米黄、羊皮纸或卷轴色作为全局页面背景。木色只属于真实场景材质。
...
**Character:** 标题宋体提供叙事与书卷气，但只出现在封面、章节名和结尾记忆卡；黑体承担所有任务、按钮、字幕、AI 对话和设置。这样既保留文化叙事气质，又不会把游戏操作做成古籍页面。
...
- 墨靛实色背景、4px 小圆角，完整环绕的低对比缝线仅可出现一次。
- 当前任务使用河青图标，完成任务使用银光勾选，未开始任务使用雾白轮廓。
